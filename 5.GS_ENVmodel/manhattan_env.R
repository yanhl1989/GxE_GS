...
#把环境因子提出来
group_split_cols <- strsplit(as.character(Snp_pos$group), "_")
Snp_pos$ENVs <- sapply(group_split_cols, function(x) x[1])
...
color <- c("#367db8","#fb9a9b","#e4191c","#fcb359","#87d5c6","#fecbe3","#be7fbc","#ab5326","#49b243","#9fb2da","#89a6af","#5581c1","#098584")

# 定义图例标签
legend_labels <- unique(Snp_pos$ENVs)

p <- ggplot(Snp_pos, aes(x = POScum, y = ratio)) +
  geom_point(aes(color = as.factor(ENVs)), alpha = 0.8, size = 1.3) +
  ...
  labs(x = "", y = "Ratio to the max feature of SNP", color = "Env factor type") + 
...
ggsave("ENV_search_select_snp_feature_manhattan.png", plot = p, width = 9, height = 6, dpi = 600)

select_snp <- as.data.frame(select_snp[,1])
colnames(select_snp)[1] <- "IID"
write.table(select_snp, file = "ENV_search_select_snp.list", col.names = F, row.names = F, quote = F )